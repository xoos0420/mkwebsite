...
class TestView(TestCase):
    ...
    def test_post_detail(self):
        ...
        self.assertEqual(post_004.get_absolute_url(), '/blog/4/')
        
        # 2. 첫 번째 포스트의 상세 페이지 테스트
        # 2.1 첫 번째 post url로 접근하면 정상적으로 작동(status cod:200)
        response = self.client.get(post_004.get_absolute_url())
        self.assertEqual(response.status_code, 200)
        soup = BeautifulSoup(response.content, 'html.parser')
        
        # 2.2 포스트 목록 페이지와 똑같은 내비게이션이 있음
        self.navbar_test(soup)
        
        # 2.3 첫 번째 포스트의 제목(title)이 웹 브라우저 탭 타이틀에 들어있음
        self.assertIn(post_004.title, soup.title.text)
        
        # 2.4 첫 번째 포스트의 제목이 포스트 영역(post_area)에 있다
        main_area = soup.find('div', id = 'main-area')
        post_area = main_area.find('div', id='post-area')
        self.assertIn(post_004.title, post_area.text)
        
        # 2.5 첫 번째 포스트의 작성자(author)가 포스트 영역에 있다
        self.assertIn(self.user_trump.username.upper(), post_area.text)
        
        # 2.6 첫 번째 포스트의 내용(content)이 포스트 영역에 있다
        self.assertIn(post_004.content, post_area.text)